...
fp_extract = ft.FilePicker(
    on_result=lambda e: _extract_gdat(Path(e.files[0].path)) if e.files else logger(t("cancelled"), color=COLOR_LOG_YELLOW)
)

# Para rebuild: primeiro seleciona o GDAT original, depois a pasta
_original_gdat_for_rebuild = None

def _on_rebuild_file_selected(e):
    global _original_gdat_for_rebuild
    if e.files:
        _original_gdat_for_rebuild = Path(e.files[0].path)
        fp_rebuild_folder.get_directory_path(dialog_title=t("select_extracted_folder"))
    else:
        logger(t("cancelled"), color=COLOR_LOG_YELLOW)

def _on_rebuild_folder_selected(e):
    global _original_gdat_for_rebuild
    if e.path:
        _rebuild_gdat(_original_gdat_for_rebuild, Path(e.path))
        _original_gdat_for_rebuild = None
    else:
        logger(t("cancelled"), color=COLOR_LOG_YELLOW)

fp_rebuild_file = ft.FilePicker(on_result=_on_rebuild_file_selected)
fp_rebuild_folder = ft.FilePicker(on_result=_on_rebuild_folder_selected)
...
# ==============================================================================
def _extract_gdat(filepath: Path):
    ...
    with open(filepath, "rb") as f:
        ...
        output_folder = base_dir / base_name
    ...
    logger(t("extraction_success", path=str(output_folder)), color=COLOR_LOG_GREEN)
    return output_folder

def _rebuild_gdat(original_gdat_path: Path, extracted_folder: Path):
    ...
    # Prepare output path
    base_dir = original_gdat_path.parent
    base_name = original_gdat_path.stem
    output_path = base_dir / (base_name + "_repacked.dat")
    ...
    with open(output_path, "wb") as out:
        ...
        for i in range(total_files):
            if i in extracted_files:
                ...
                file_path = extracted_folder / fname
    ...
    logger(t("repack_saved", path=str(output_path)), color=COLOR_LOG_GREEN)
    return output_path
...
def register_plugin(log_func, option_getter, host_language="pt_BR", page=None):
    ...
    if host_page:
        host_page.overlay.extend([fp_extract, fp_rebuild_file, fp_rebuild_folder])
        host_page.update()
