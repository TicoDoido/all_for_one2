...
# ==============================================================================
# FUNÇÃO PRINCIPAL DE EXTRAÇÃO (ADAPTADA PARA USAR LOGGER)
# ==============================================================================
def extract_pack(filepath):
    try:
        if not os.path.exists(filepath):
            logger(t("err_file_not_found", path=filepath), color=COLOR_LOG_RED)
            return
        ...
        logger(t("extracting_to", path=extracted_dir), color=COLOR_LOG_YELLOW)
        ...
        logger(t("log_json_written", json_path=json_path), color=COLOR_LOG_GREEN)
        logger(t("log_extracted_folder", folder=extracted_dir), color=COLOR_LOG_GREEN)
...
# ==============================================================================
# AÇÃO DO COMANDO (SEM THREADING)
# ==============================================================================
def action_extract():
    path = pick_file_topmost(t("select_image_file"), [(t("image_bin"), "*.bin"), (t("all_files"), "*.*")])
    if not path:
