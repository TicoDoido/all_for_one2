...
fp_extract = ft.FilePicker(
    on_result=lambda e: read_binary_file(e.files[0].path) if e.files else logger(t("cancelled"), color=COLOR_LOG_YELLOW)
)
fp_rebuild = ft.FilePicker(
    on_result=lambda e: rebuild_binary_file(
        e.files[0].path,
        os.path.splitext(e.files[0].path)[0] + "_MOD" + os.path.splitext(e.files[0].path)[1],
        os.path.splitext(e.files[0].path)[0]
    ) if e.files else logger(t("cancelled"), color=COLOR_LOG_YELLOW)
)
...
# ==============================================================================
# AÇÕES DOS COMANDOS (USAM OS FILEPICKERS)
# ==============================================================================
