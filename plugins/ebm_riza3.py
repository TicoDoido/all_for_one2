...
def pick_file_topmost(title, file_types):
    """Cria uma janela Tk invisível, força ela pro topo e abre diálogo de arquivo único."""
    root = tk.Tk()
    root.withdraw()
    root.wm_attributes('-topmost', 1)
...
# ==============================================================================
# AÇÕES DOS COMANDOS (ADAPTADAS PARA USAR LOGGER E SEM THREADING)
# ==============================================================================

def action_extract():
    paths = pick_files_topmost(t("select_ebm_files"), [(t("ebm_files"), "*.ebm"), (t("all_files"), "*.*")])
    if not paths:
    ...
    for idx, filepath in enumerate(paths, 1):
        ...
        try:
            ...
            out_txt = Path(filepath).with_suffix(".txt")
...
def action_import():
    paths = pick_files_topmost(t("select_txt_files"), [(t("text_files"), "*.txt"), (t("all_files"), "*.*")])
    if not paths:
        logger(t("cancelled"), color=COLOR_LOG_YELLOW)
        return
    ...
    for idx, filepath in enumerate(paths, 1):
        logger(t("processing_file", current=idx, total=total, name=os.path.basename(filepath)), color=COLOR_LOG_YELLOW)
        try:
            txt_path = Path(filepath).resolve()
            base = txt_path.stem
            ...
            for idx_ev, ev in enumerate(ebm.events):
                if idx_ev in mapping:
                    ev.writeEventText(mapping[idx_ev])
                    replaced += 1

            out_path = ebm_candidate
            ebm.save(str(out_path))
            logger(t("import_completed", replaced=replaced, path=str(out_path)), color=COLOR_LOG_GREEN)
        except Exception as e:
            logger(t("unexpected_error", error=str(e)), color=COLOR_LOG_RED)
...
def action_extract_gz():
    paths = pick_files_topmost(t("select_gz_files"), [(t("gz_files"), "*.gz"), (t("all_files"), "*.*")])
    if not paths:
        logger(t("cancelled"), color=COLOR_LOG_YELLOW)
        return
...
def action_compress_gz():
    ...
    for idx, filepath in enumerate(paths, 1):
        ...
        try:
            p = Path(filepath).resolve()
            out_path = p.with_name(p.name + '.gz')  # append .gz
