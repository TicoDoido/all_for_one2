...
fp_extract = ft.FilePicker(
    on_result=lambda e: _extract_arc(Path(e.files[0].path)) if e.files else logger(t("cancelled"), color=COLOR_LOG_YELLOW)
)

fp_rebuild = ft.FilePicker(
    on_result=lambda e: _rebuild_arc(Path(e.files[0].path)) if e.files else logger(t("cancelled"), color=COLOR_LOG_YELLOW)
)
...
# ==============================================================================
def _extract_arc(arc_path: Path):
    try:
        ...
        output_dir = arc_path.parent / arc_path.stem
...
def _rebuild_arc(arc_path: Path):
    try:
        ...
        with arc_path.open('r+b') as f:
            ...
            new_data = []
            f.seek(data_start)
            extracted_dir = arc_path.parent / arc_path.stem
...
def action_extract():
    fp_extract.pick_files(
        allowed_extensions=["arc"],
        dialog_title=t("select_arc_file")
    )
